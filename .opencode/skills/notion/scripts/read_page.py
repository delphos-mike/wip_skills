...
from notion_utils import api_call, extract_rich_text, get_all_blocks, load_api_key, parse_notion_id
...
def block_to_markdown(block: dict[str, Any], indent_level: int = 0) -> str:
    ...
    if block_type == "paragraph":
        ...
        # Render table from children (table_row blocks).
        # get_all_blocks already fetched children recursively.
        rows = block.get("children", [])
        has_header = block_content.get("has_column_header", False)
        for row_idx, row_block in enumerate(rows):
            cells_data = row_block.get("table_row", {}).get("cells", [])
            cells = [extract_rich_text(cell) for cell in cells_data]
            lines.append(f"{indent}| {' | '.join(cells)} |")
            if row_idx == 0 and has_header:
                lines.append(f"{indent}| {' | '.join('---' for _ in cells)} |")
        # Return early — don't process children generically (they're table_rows)
        return "\n".join(lines)
...
def block_to_text(block: dict[str, Any], indent_level: int = 0) -> str:
    ...
    if block_type == "table":
        # Render table rows as tab-separated values
        rows = block.get("children", [])
...
def main():
    parser = argparse.ArgumentParser(
        description="Read Notion page content", formatter_class=argparse.RawDescriptionHelpFormatter, epilog=__doc__
    )
