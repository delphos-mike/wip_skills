...
def main():
    parser = argparse.ArgumentParser(description='Find and link page references',
                                     formatter_class=argparse.RawDescriptionHelpFormatter,
                                     epilog=__doc__)
    parser.add_argument('page_id', help='Page ID')

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('--find-references', action='store_true',
                      help='Find potential page references')
    group.add_argument('--link', nargs=2, metavar=('QUERY', 'TARGET_PAGE_ID'),
            ...
            for ref_text, ref in unique_refs.items():
                ...
                if matches:
                    ...
                    for match in matches[:3]:
                        title = match.get('properties', {}).get('title', {}).get('title', [{}])[0].get('plain_text', 'Untitled')
